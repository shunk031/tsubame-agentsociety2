...
| モデル | ラン数 | 名前が壊れたラン |
| --- | --- | --- |
| `Qwen3.6-35B-A3B-FP8` | 8 | **0** |
| `Qwen3.5-4B` / 16 体 | 6 | 3 |
| `Qwen3.5-4B` / 128 体 | 1 | 1（複数件） |

- 壊れ方は一様ではない
  - `"3"` `"16"` `"2055"` — 名前の数字部分だけ、あるいは無関係な数
  - `"Agent_60"` `"Agent 3"` `"agent16"` — 区切りや大小の崩れ
  - `"cheese"` `"true"` — 名前と無関係な語。書き損じでは説明できない
...
- **128 体でも埋まらない。** 1145 サンプル中 `Waiting` がゼロでないのは 2 回だけ
  ...
  | 体数 | `Running` の定常 | `Waiting` | KV 最大 |
  | --- | --- | --- | --- |
  | 16 | 1〜3 本 | 0 | 0.7% |
  | 128 | 1〜4 本（稀に 32〜46） | ほぼ 0 | 13.3% |

  - ➜ 頭数を 8 倍にしても飽和しない。GPU を増やす判断には、まず飽和させる方法が要る

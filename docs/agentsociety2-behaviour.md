...
GPU が埋まらない理由も、ステップ末尾で電力が落ちる理由も、ここに全部ある。
出典は `agent/runner.py` の docstring と `society/society.py` の設計メモ。

```mermaid
flowchart TB
  subgraph tick["1 ティック"]
    direction TB
    D["ドライバ<br/>エージェント id を BATCH_SIZE で分割"]
    D --> T1["Ray Task 1<br/>asyncio.gather"]
    D --> T2["Ray Task 2<br/>asyncio.gather"]
    D --> Tn["Ray Task N<br/>asyncio.gather"]
    T1 --> B(("await 全タスク<br/>＝バリア"))
    T2 --> B
    Tn --> B
    B --> E["env を進める"]
    E --> C["時計を進める"]
...
ドライバはエージェント**オブジェクト**を持たない。id だけを持ち、毎ティック
`step_agent_batch` Ray Task を投げ、**全部の完了を待ってから** env と時計を進める。

この `await` がバリアで、1 体でも終わらなければ次に進めない。**これは社会シミュレーションの
要件**であって、外すべき非効率ではない。同一ティック内で全員が行動するからこそ、
全エージェントが同じ時刻の世界を観測したと言える。外せば、あるエージェントが数ステップ先の
世界を見ながら別のエージェントは過去を見ることになり、相互作用の記録として成立しない。

以下は「バリアを保ったまま GPU をどう埋めるか」の話であって、バリアを外す話ではない。

並列は 2 層ある。タスクが Ray ワーカーに分散し、さらに 1 タスクの中で
エージェントが `asyncio.gather` で同時に走る。全体の同時実行数は
ディスパッチャの AdaptiveSemaphore が上限を決める。

### 1 エージェントは 1 リクエストではない

```mermaid
sequenceDiagram
  participant A as エージェント
  participant S as AdaptiveSemaphore
  ...
  S->>V: 行動を書く
  V-->>A: 応答
  Note over A: 連鎖の回数はエージェントごとに違う
```

`runner.py` の言葉では each agent is LLM-bound, **a chain of awaited LLM/tool calls**。
連鎖の長さは個体差が大きく、それがそのままティックの長さになる。

### なぜ末尾で GPU が空くか

vLLM は終わったリクエストから個別に返す。バッチ単位で待ち合わせる箇所はない。
なので連鎖を終えたエージェントから抜けていき、同時実行数は**開始直後が最大で単調に減る**。

```
ティック開始   Running: 128   全員が連鎖の 1 本目を出す
中盤           Running:  60   連鎖の短い個体が抜けていく
終盤           Running:   2   連鎖の長い個体だけ残る
               ^^^^^^^^^^^^   残り 126 体はここで何もせず待っている
```

実測（128 体 / H100 x4）の推移。左が開始直後:

```
128 128 126 126 128 127 128 127 126 127 ... 124 123 120 118 ...
```

- **谷は仕様**であってバグではない。バリアがある限り、末尾は必ず細る
- 1 回の生成長（`--override-generation-config` の `max_new_tokens`）を切っても
  連鎖の回数は減らないので、末尾はほとんど縮まない
- 連鎖の回数そのものを制限すれば末尾は縮むが、**エージェントの思考を途中で打ち切る**ことになる。
  シミュレーションの妥当性と引き換えなので、性能のためだけに触る設定ではない
- ➜ エージェント数を増やすと連鎖長のばらつきが均され、谷が浅くなる。
  実測では 4 GPU に対し 128 体は供給不足で、512 体で毎時イベントが 333 → 891 に増えた

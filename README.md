...
**GPU を増やすほど 1 枚あたりの電力は下がる。埋めたいなら枚数を減らす**

- テンソル並列はモデルを分割するので、1 枚が読む重みも 1 枚が担う計算密度も枚数分だけ減る。
  [arXiv:2504.17674](https://arxiv.org/pdf/2504.17674) が per-device 電力の低下として測っている
- 手元の実測も同じ（同一ベンチ・同一並列度 32・Qwen3.6-27B）
...
- 128 体・SocialMediaSpace での比較

  | | 平均電力 | 80% 以上の時間 |
  | --- | --- | --- |
  | H100 x4（TP） | 64〜67% | 22〜29% |
  | **H100 x1** | **82%** | **73%** |

- 4 枚だと `Running: 38, Waiting: 90`。枠はあるのに供給が足りず、ステップ境界ごとに
  数分単位で完全に止まる区間ができる。1 枚だと `Running: 107, Waiting: 11` で行列が絶えない
- ➜ **エージェント数に対して GPU が多すぎると電力は上がらない。** 設定を詰めても埋まらない。
  モデルが 1 枚に載るなら、まず枚数を減らして密度を上げる

**同じ並列度でもクライアントが届けていないことがある**

- ベンチの並列度を上げても `gen_tok/s` が動かないとき、サーバが飽和したのか
  クライアントが詰まったのかは、数字だけでは区別がつかない
- **見るのは vLLM の `Waiting`。** `Waiting: 0` のまま `gen_tok/s` が頭打ちなら、
  サーバは待たされていない＝リクエストが届いていない
- 実測: `bench-8688255` は並列度 64/128/256 を指定したのに、エンジンが見た
  `Running` の最大は **32**、全サンプルで `Waiting: 0` だった。
  「2,790 tok/s が上限」という結論はクライアント側の頭打ちを測ったもので、誤り
- 原因は `asyncio` の既定エグゼキュータ `min(32, cpu_count + 4)`。
  `scripts/bench_vllm.py` は並列度に合わせてプールを張るので現在は起きない
